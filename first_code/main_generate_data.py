...
# Parámetros iniciales (deben ser definidos anteriormente en tu script)
xcoord = [2, 2]
ycoord = [1.25, 1.25]
...
params = {
    ...
    'num_time_bins': 154,  # Este valor puede ser calculado dinámicamente si es necesario
}
...
for xx in range(len(xcoord)):
    ...
    plt.figure()
    plot_scene_dual_training_selfOcc(laser_pos, objects, params)
    plotFlag = 0
    ...
    pixel_x = np.linspace(camera_FOV_center[0] - camera_FOV / 2 + camera_FOV / (2 * cam_pixel_dim),
                          camera_FOV_center[0] + camera_FOV / 2 - camera_FOV / (2 * cam_pixel_dim), cam_pixel_dim)
    pixel_y = np.linspace(camera_FOV_center[1] - camera_FOV / 2 + camera_FOV / (2 * cam_pixel_dim),
                             camera_FOV_center[1] + camera_FOV / 2 - camera_FOV / (2 * cam_pixel_dim), cam_pixel_dim)

    pixel_t = np.linspace(0, (params['num_time_bins'] - 1) * bin_size, params['num_time_bins'])

    X, Y = np.meshgrid(pixel_x, pixel_y)
    cam_pos = np.vstack([X.ravel(), Y.ravel(), np.zeros(cam_pixel_dim**2)]).T

    X_ind, Y_ind = np.meshgrid(range(1, cam_pixel_dim + 1), range(1, cam_pixel_dim + 1))
    ...
    floor_pixel_width = camera_FOV / cam_pixel_dim
    ...
    for sp in range(spix):
        ...
        if np.sum(noc) > 0:
            ...
            dot3 = np.maximum(0, np.sum(laser_normal * -lps / d1[:, np.newaxis], axis=1))
            ...
            coord = (arrival_bin - 1) * cam_pixel_dim**2 + (cam_pos_ind[noc, 0] - 1) * cam_pixel_dim + cam_pos_ind[noc, 1]
    ...
    filename = f"Video_{int(bin_size * 1e12)}ps_ThinFacet_pos_{xx}.mat"
